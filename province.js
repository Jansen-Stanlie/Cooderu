...
function getCitiesProvince(province) {
	let city = [];
	for (let i = 0; i < provinces.length; i++) {
		if (province == provinces[i].name) {
			console.log(provinces[i].id);
			let id = provinces[i].id;
			for (let j = 0; j < regencies.length; j++) {
				if (regencies[j].province_id == id) {
					city.push(regencies[j].name);
				}
			}
		}
	}
...
// getCitiesProvince("PAPUA");

function getCitiesContainText(province) {
	let cities = [];
	provinces.forEach((prov) => {
		if (prov.name.includes(province)) {
			let id = prov.id;
...
// getCitiesContainText("SUMATERA");

function getProvincebyCities(City) {
	let provs = [];
	regencies.forEach((cities) => {
		if (cities.name.toLowerCase() == City.toLowerCase()) {
			console.log(true);
			let prov_id = cities.province_id;
			console.log(City.toLowerCase());
			provinces.forEach((prov) => {
				if (prov.id == prov_id) {
					provs.push(prov.name);
				}
			});
		}
	});
...
// getProvincebyCities("KoTA MeDaN");

function getRegenciesByCount(city) {
	let cityCount = city.split(" ");
	let citiesCount = [];
	console.log(cityCount.length);
	regencies.forEach((cities) => {
		if (city.toLowerCase() == cities.name.toLowerCase()) {
			regencies.forEach((citiess) => {
				if (cityCount.length == citiess.name.split(" ").length) {
					citiesCount.push({
						id: citiess.id,
						Kota: citiess.name,
						nama_Lain: citiess.alt_name,
					});
				}
			});
		}
	});
	citiesCount.length == 0
		? console.log("City not listed in Regencies")
		: console.log(citiesCount);
}
getRegenciesByCount("KABUPATEN MAMBERAMO RAYA");
